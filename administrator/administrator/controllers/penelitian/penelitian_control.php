...
if($model=='penelitian' AND $method=='tambah' ){
 	...
 	if(isset($_POST['simpan'])){
		...
		$link = $libs->changeLink($judul_penelitian);
		...
		$tujuan_penilitian = $_POST['tujuan_penilitian'];

		$manfaat_penilitian = $_POST['manfaat_penilitian'];

		$hasil_penilitian = $_POST['hasil_penilitian'];

		$durasi_penelitian = $_POST['durasi_penelitian'];

		$grant_abstrak = $_POST['grant'];
		...
		if(!empty($_FILES['file']['name'])){
			...
			$gambar = $libs->uploadImageToFolder('../../../images/content/penilitian/',$_FILES['file']);	//upload
		 	
			$thumbnail = $libs->uploadImageToFolderThumbnail('../../../images/content/penilitian/',$gambar);	//thumbnail
		...
		foreach($hasil as $hasil){ //eksekusi
			...
			$penelitian->insertPenelitianDB($judul_penelitian,$ketua_penelitian,'','','','',$tahun_penelitian,$sumber_dana,$abstrak, $gambar,$tujuan_penilitian,$manfaat_penilitian,$hasil_penilitian,$grant_abstrak,$nama_lab,$kata_kunci,$link,$durasi_penelitian,$db);
	...
	###
 	if(isset($_POST['simpan'])){
		...
		if(!empty($_FILES['file']['name'])){
			...
			$libs-> hapusGambarSpesific("../../../images/content/penilitian/", $_POST['file']);
			
			$libs-> hapusGambarSpesific("../../../images/content/penilitian/thumbnails/", $_POST['file']);
			// $gambar = $libs->uploadImage($_FILES['file']);	// ke folder upload
			$gambar = $libs->uploadImageToFolder('../../../images/content/penilitian/',$_FILES['file']);	//upload
		...
		########## jika penambahan oleh mahasiswa
			...
			$penelitian->updatePenelitianDB($id_penelitian,$judul_penelitian,$ketua_penelitian,'','','','',$tahun_penelitian,$sumber_dana,$abstrak, $gambar,$tujuan_penilitian,$manfaat_penilitian,$hasil_penilitian,$grant_abstrak,$nama_lab,$kata_kunci,$durasi_penelitian,$db);
...
else if($model=='penelitian' AND $method=='hapus_gambar' ){

	$id = $_GET['id'];
	
	$data = $penelitian->getPenilitianById($id);
	// var_dump($data);
	// $libs->hapusFile($data['gambar']);

 	$libs-> hapusGambarSpesific("../../../images/content/penilitian/", $data['abstrak_img']);
	
	$libs-> hapusGambarSpesific("../../../images/content/penilitian/thumbnails/", $data['abstrak_img']);

	var_dump($penelitian->hapusGambarArtikelById($id));
	echo 11;
	
	echo"<script> alert('Menghapus gambar'); </script>";
	
	header("location:".URL."penelitian/edit/".$id."");
}
else{
