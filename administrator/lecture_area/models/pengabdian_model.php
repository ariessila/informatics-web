...
class pengabdian_model{
	...
	public function getPengabdian($a,$b,$c){

		$query = $this->db->prepare("SELECT distinct tahun,pengabdian.judul_pengabdian,ketua,sumber_dana,pengabdian.id FROM  `pengabdian` LEFT JOIN `pengabdian_anggota` ON pengabdian_anggota.id_pengabdian = pengabdian.id where ketua = :c  OR `nama_dosen` = :d order by pengabdian.id DESC limit :a,:b");
