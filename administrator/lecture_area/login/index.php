...
$user = $dosen->getDosenById($username); // get dosen sama password
...
if ($pass == $user['password']){
 	...
 	$count=$dosen->countDosenById($username);// login
	

}else{

	header('location:'.ROOT.'id/page/18/dosen.html?login=false');
...
if ($count > 0){
	  @session_start();
	  // include "timeout.php";
	  $_SESSION['nip_dosen']    			= $user['nip'];
	  $_SESSION['nama_dosen'] 		= $user['nama_dosen'];
	  $_SESSION['password_dosen']   = $user['password'];
	  $_SESSION['login_dosen'] = 1;

	  // session timeout
	  
	  $libs->timer();
	
	  $_SESSION['timeout_dosen'];
			...
			<div class='panel panel-default' id='panel-default'>	
				...
				<form role='form' action='' method='POST' id='login' onsubmit='return sub();'>
					...
					<div class='form-group'>
						...
						<label><a href='#' id='forgot'>lupa password?</a></label>
					</div>
