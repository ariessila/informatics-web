...
class Publikasi extends Public_Controller {
    ...
    public function index()
    {
        ...
        $data = $this->crud->gw($this->table, array('id_halaman' => 9))[0];
        $pengaturan = $this->crud->gw('pengaturan', array('id_pengaturan' => 1))[0];

        $q = $this->input->get('q', TRUE) ? $this->input->get('q', TRUE) : NULL;
        $cari_query = cari_query($q, array('id','judul', 'deskripsi', 'oleh', 'tahun'));
        
        $config['total_rows'] = $this->crud_fakultas->cw('publikasi_dosen',$cari_query);
        ...
        $halaman = $this->crud_fakultas->gwlo('publikasi_dosen' ,$cari_query, $config['per_page'], $offset, 'tahun DESC');
        ...
        $data = array(  'title'     => $data->{'nama_'.$this->lang},
                        ...
                        'isi'       => 'homepage/publikasi/view_#'.$this->lang);
